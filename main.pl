...
parse(Input,Tree):- 
			...
			(mainDebug-> write("parse: Derivation Tree blank: "),writeln(ParseTree);true),
			workSpace(ParseTree,TreeSyn),
			(mainDebug-> write("parse: Derivation Tree syntax full: "),writeln(TreeSyn);true),
			lappend(TreeSyn,TreeSem),
			(mainDebug-> write("parse: Derivation Tree semantics full: "),writeln(TreeSem);true),
			betaRoot(TreeSem,Tree),
			(mainDebug-> write("parse: Derivation Tree semantics beta-reduced: "),writeln(Tree);true).
			%tree_painter(Tree).
